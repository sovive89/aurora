...
def chat():
    ...
    try:
        # URL do WebSocket para a API de Conversational AI da ElevenLabs
        # Note que o endpoint é diferente e usa ws:// ou wss://
        ws_url = f"wss://api.elevenlabs.io/v1/convai/agents/{ELEVENLABS_AGENT_ID}/stream"
        
        # Parâmetros de query para a conexão WebSocket
        params = {
        ...
        full_ws_url = ws_url + "?" + "&".join([f"{k}={v}" for k, v in params.items()])

        ws = websocket.create_connection(full_ws_url)

        # Enviar a mensagem do usuário via WebSocket
        # A estrutura da mensagem WebSocket é diferente da requisição HTTP POST
        ws_message = {
            "text": user_message,
            "try_trigger_tool": True, # Tentar acionar ferramentas se configuradas
            "user_id": session_id # Para rastrear a sessão
        }
            ...
            while True:
                try:
                    ...
                    if message:
                        ...
                        if "audio" in data: # Se a mensagem contiver dados de áudio
                            yield base64.b64decode(data["audio"])
                        elif "is_final" in data and data["is_final"]: # Fim da conversa
                            break
