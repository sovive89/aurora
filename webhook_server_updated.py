...
def chat():
    ...
    try:
        ...
        signed_url_endpoint = f"https://api.elevenlabs.io/v1/convai/conversation/get-signed-url"
        headers = {
            "xi-api-key": ELEVENLABS_API_KEY,
            "Content-Type": "application/json"
        }
        
        # Adicionar parâmetros como JSON no corpo da requisição em vez de query params
        signed_url_payload = {
        ...
        print(f"Headers: {json.dumps({'xi-api-key': '****', 'Content-Type': 'application/json'})}")
        print(f"Payload: {json.dumps(signed_url_payload)}")
        ...
        print(f"Resposta da URL assinada - Status: {signed_url_response.status_code}")
        print(f"Resposta da URL assinada - Headers: {dict(signed_url_response.headers)}")
        
        try:
            response_json = signed_url_response.json()
            print(f"Resposta da URL assinada - Body: {json.dumps(response_json)}")
        except:
            print(f"Resposta da URL assinada - Body (não é JSON): {signed_url_response.text}")
        ...
        # Configurar opções do WebSocket para melhor tratamento de erros
        ws_options = {
            "timeout": 30,  # 30 segundos de timeout
        }
        
        # Tentar estabelecer conexão WebSocket com retry
        max_retries = 3
        retry_count = 0
        ws = None
        ...
        while retry_count < max_retries:
            try:
                print(f"Tentativa {retry_count + 1} de conexão WebSocket...")
                ws = websocket.create_connection(ws_url, **ws_options)
                print("Conexão WebSocket estabelecida com sucesso!")
                break
            except Exception as e:
                retry_count += 1
                print(f"Falha na tentativa {retry_count} de conexão WebSocket: {e}")
        ...
        init_message = {
            ...
            "conversation_initiation_client_data": {
                ...
                "model_id": "eleven_multilingual_v2", # Modelo multilíngue para suporte ao português
                "sample_rate": 44100,
                "user_id": session_id
            }
        }
                    ...
                    try:
                        ...
                        if message:
                            ...
                            if message_type == "audio" and "audio_base_64" in data.get("audio_event", {}):
                                ...
                                if "text" in response_event:
                                    response_text += response_event["text"]
                                if response_event.get("is_final"):
                        ...
                        print(f"Erro ao decodificar JSON da mensagem WebSocket: {e}")
                        continue
                    except Exception as e:
        ...
        try:
            ...
            tts_json_data = {
                "text": "Desculpe, tive um problema de conexão. Vou apenas repetir o que você disse: " + user_message,
                "model_id": "eleven_multilingual_v2",
                "stream": True
            }
            
            print(f"Usando fallback TTS após erro WebSocket com a mensagem: {tts_json_data['text']}")
            response = requests.post(tts_url, headers=headers, json=tts_json_data, stream=True)
            response.raise_for_status()
...
def health_check():
    ...
    Útil para monitoramento e para garantir que o serviço está funcionando.
    """
    return jsonify({
        "status": "healthy",
